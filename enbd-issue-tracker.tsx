...
const lsSet = async (key, val, shared=true) => {
  try { await window.storage.set(key, JSON.stringify(val), shared); } catch(e) { console.error("storage write failed", key, e); }
};
...
function LiveDataPanel({c,isFP}) {
  ...
  const load = useCallback(async()=>{
    ...
    try {
      let sfN=[], jiraT=[];
      try {
        const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:"Query CData MCP. Return ONLY valid JSON: {\"comments\":[{\"body\":\"...\",\"date\":\"...\",\"isPublic\":true}]}",messages:[{role:"user",content:`SELECT CommentBody,CreatedDate,IsPublished FROM [Salesforce].[Salesforce].[CaseComment] WHERE [ParentId] IN (SELECT [Id] FROM [Salesforce].[Salesforce].[Case] WHERE [CaseNumber]='${c.case}') ORDER BY CreatedDate DESC LIMIT 5`}],mcp_servers:[{type:"url",url:"https://mcp.cloud.cdata.com/mcp",name:"cdata"}]})});
        const d=await r.json(); if(!d.type) { const t=(d.content||[]).filter(b=>b.type==="text").map(b=>b.text).join(""); sfN=JSON.parse(t.replace(/```json|```/g,"").trim()).comments||[]; }
      } catch(e){}
      setSfNotes(sfN);
      try {
        const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:"Query CData MCP. Return ONLY valid JSON: {\"issues\":[{\"key\":\"...\",\"summary\":\"...\",\"status\":\"...\",\"assignee\":\"...\",\"updated\":\"...\"}]}",messages:[{role:"user",content:`SELECT [Key],[Summary],[StatusName],[AssigneeDisplayName],[Updated] FROM [Jira].[Jira].[Issues] WHERE [ProjectKey]='GS' AND [Summary] LIKE '%ENBD%' AND [Updated]>='2025-01-01' ORDER BY [Updated] DESC LIMIT 5`}],mcp_servers:[{type:"url",url:"https://mcp.cloud.cdata.com/mcp",name:"cdata"}]})});
        const d=await r.json(); if(!d.type) { const t=(d.content||[]).filter(b=>b.type==="text").map(b=>b.text).join(""); jiraT=JSON.parse(t.replace(/```json|```/g,"").trim()).issues||[]; }
      } catch(e){}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",borderTop:`1px solid ${C.border}`}}>
      <div style={{padding:"12px 14px",borderRight:`1px solid ${C.border}`,background:"#FFF9F0"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          ...
          {loaded&&<button onClick={()=>{setLoaded(false);setSfNotes(null);setJiraTickets(null);setFailed(false);}} style={{fontSize:8,padding:"2px 7px",borderRadius:4,border:`1px solid ${C.amberBd}`,background:C.amberBg,color:C.amber,cursor:"pointer",fontFamily:"monospace"}}>↻</button>}
        </div>
          ...
          <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,padding:"10px 14px",display:"flex",gap:12,alignItems:"flex-start"}}>
            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                <span style={{fontFamily:"monospace",fontSize:9,fontWeight:700,color:C.navy}}>
                  <a href={`${SF_BASE}/${SEED.find(s=>s.case===entry.caseNum)?.sfId||entry.caseNum}/view`} target="_blank" rel="noopener noreferrer" style={{color:C.navy,textDecoration:"none",borderBottom:`1px dashed ${C.navy}`}}>{entry.caseNum}</a>
                </span>
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try { const cu=await window.storage.get(SK.user,false); if(cu?.value&&cu.value!=="null") setUser(JSON.parse(cu.value)); } catch(e){}
  ...
  const persistAll = useCallback(async(ov,cl,co) => {
    await lsSet(SK.overrides,ov);
    await lsSet(SK.changelog,cl);
    await lsSet(SK.comments, co);
  },[]);
  ...
  const saveEngineering = useCallback(async(caseNum,val)=>{
    ...
    const seed=SEED.find(s=>s.case===caseNum)||{};
  ...
  const saveEdit = useCallback(async(caseNum)=>{
    ...
    setSaveStatus(p=>({...p,[caseNum]:"saved"}));
    setTimeout(()=>setSaveStatus(p=>({...p,[caseNum]:null})),2000);
  },[overrides,changelog,comments,editVal,user,logChange,persistAll]);
  ...
  const merged=SEED.map(c=>({...c,...((overrides||{})[c.case]||{})}));
  ...
  const filtered=merged.filter(c=>{
    ...
    return filterByTab(c,tab)&&(!s||c.case.includes(s)||(c.subject||"").toLowerCase().includes(s)||(c.owner||"").toLowerCase().includes(s)||(c.status||"").toLowerCase().includes(s));
  });
  ...
  if(!user) return <RoleGate onSelect={setUser}/>;
      ...
      <div style={st.hdr}>
        <div style={st.hi}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <button onClick={async()=>{try{await window.storage.set(SK.user,"null",false);}catch(e){}setUser(null);}} style={{padding:"4px 10px",borderRadius:5,border:"1px solid rgba(255,255,255,0.15)",background:"transparent",color:"rgba(255,255,255,0.4)",fontSize:9,cursor:"pointer"}}>Not you?</button>
          </div>
        </div>
      ...
      <div style={{padding:"14px 16px 60px"}}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:10,alignItems:"center"}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by case #, subject, owner or status..."
            style={{flex:1,minWidth:200,padding:"6px 12px",borderRadius:6,border:`1px solid ${C.border}`,fontSize:11,outline:"none",background:C.surface}}/>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:10.5}}>
            <thead>
              <tr style={{background:C.navyDk}}>
                ...
                <th style={{...st.th,width:90}}>Case #</th>
                <th style={{...st.th,minWidth:200}}>Subject</th>
                <th style={{...st.th,minWidth:115}}>Status</th>
                <th style={{...st.th,width:55}}>Sev</th>
                <th style={{...st.th,minWidth:90}}>Component</th>
                <th style={{...st.th,minWidth:90}}>Owner</th>
                <th style={{...st.th,width:72}}>Opened</th>
                <th style={{...st.th,width:65}}>Closed</th>
                <th style={{...st.th,minWidth:140}}>Resolution Notes</th>
                ...
                return [
                  <tr key={c.case} style={{background:bg,cursor:"pointer"}} onClick={()=>setExpanded(p=>({...p,[c.case]:!p[c.case]}))}>
                    ...
                    <td style={st.td}>
                      ...
                      {c.isUpgrade&&<span style={{marginLeft:4,fontSize:8,color:C.orange}}>🔺</span>}
                      {ov&&<span style={{marginLeft:4,fontSize:8,color:saved?"#16A34A":C.teal}}>{saved?"✓":"✎"}</span>}
                    </td>
                    ...
                    <tr key={c.case+"-desc"}>
                      <td colSpan={isFP?13:12} style={{padding:0,borderRight:"none"}}>
                        <div style={{padding:"12px 16px 12px 44px",background:"#F0F9FF",borderBottom:`1px solid ${C.border}`}}>
                          ...
                          {c.latestSfNote&&<div style={{marginTop:8,padding:"7px 10px",background:"#FFFBEB",border:`1px solid ${C.amberBd}`,borderRadius:5,fontSize:10,color:C.amber,lineHeight:1.5}}><strong>☁ Latest SF Note:</strong> {c.latestSfNote}</div>}
                          {c.suggestion&&(
        ...
        <div style={{marginTop:10,fontSize:9,color:C.inkSoft,fontFamily:"monospace",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <span>Forcepoint DSPM · Emirates NBD · AccountId: 0015f00000R7aqFAAR · 44 cases · Synced 13 Apr 2026</span>
          <span>Storage: {SK.overrides} · All changes shared · {ready?"✓ Ready":"⌛ Loading..."}</span>
        </div>
      </div>
    </div>
  );
